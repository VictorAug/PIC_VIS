...
public class Visio {
	...
	private void addGraphicsPanel() {
		...
		sliderPanel.setLayout(new MigLayout("", "0[grow]0",
				"0[25px:n:25px][0px:n:n]"));
	...
	private void addEspectroTab() {
		...
		espectroPanel.setLayout(new MigLayout("",
				"0[245px:245px:245px,grow]0[grow]0[142px:142px:142px]0",
	...
	private void addEspectroFieldset() {
		...
		btnParar.addActionListener((ActionEvent actionEvent) -> {
			if ("disable".equals(actionEvent.getActionCommand())) {
				...
				System.out.println("Time elapsed: " + time
						+ ". Number of Charts: " + chartCollection.count());
			} else {
				btnAdquirir.setEnabled(false);
				btnParar.setEnabled(true);
		...
		rdbtnUnico.addActionListener((ActionEvent actionEvent) -> {
			if ("Unico".equals(actionEvent.getActionCommand())) {
				btnParar.setEnabled(false);
				setReadOnce(true);
				btnAdquirir.setEnabled(true);
			} else {
		...
		rdbtnContinuo.addActionListener(actionEvent -> {
			if ("Continuo".equals(actionEvent.getActionCommand())) {
				btnParar.setEnabled(true);
				setReadOnce(false);
				btnParar.setEnabled(true);
			} else {
		...
		opcoesFieldSet.add(rdbtnNewRadioButton_1,
		...
		rdbtnNewRadioButton_1.addActionListener(actionEvent -> {
			if ("Counts".equals(actionEvent.getActionCommand()) && flag) {
		...
		opcoesFieldSet.add(rdbtnNewRadioButton_2,
				"cell 1 3,alignx center,growy");
		...
		rdbtnNewRadioButton_2.addActionListener((ActionEvent actionEvent) -> {
			if ("mV".equals(actionEvent.getActionCommand()) && !flag) {
		...
		slider.addChangeListener((ChangeEvent e) -> label.setText(""
				+ slider.getValue()));
		...
		slider_1.addChangeListener((ChangeEvent e) -> label_1.setText(""
				+ slider_1.getValue()));
	...
	private class Connector implements Runnable {
		...
		public void run() {
			while (true) {
				...
				if (harvester != null)
					if (harvester.tryConnection(((String) commComboBox
							.getSelectedItem()))) {
